...
function saveChange(file, res) {
    if (res.type === 'lyric') {
        ...
        const next = question(`\nWARN: Save 'other' type files will result in metadata and subtitle effect loss. Do you confirm the operation(Y/n)? `)
        if (next === '' || next === 'Y' || next === 'y') {
            fs.writeFileSync(`${__dirname}/cache/LYRICS-CON-TEMP.lrc`, res.lrcData, 'utf8')
            execSync(`ffmpeg -loglevel quiet -i "${resolve(__dirname, './cache/LYRICS-CON-TEMP.lrc')}" "${resolve(__dirname, './cache/LYRICS-TEMP.srt')}" -y`)
            let mapCliStr = ''
            if (res.streamIndex > 0) 
                for (let i = 0; i <= res.streamIndex; i++) {
                    if (i === res.streamIndex) 
                        break
                    mapCliStr += `-map 0:${i} `
                }
            const { ext } = parse(file)
            execSync(`ffmpeg -loglevel quiet -i "${file}" -i "${resolve(__dirname, './cache/LYRICS-TEMP.srt')}" ${mapCliStr} -map 1:0 -c copy "${file}.temp${ext}" -y`)
            fs.rename(file, `${__dirname}/cache/SAVE-TEMP`, (err) => { 
                if (err) error(err.message)
                fs.renameSync(`${file}.temp${ext}`, file)
                fs.writeFileSync(`${__dirname}/cache/SAVE-FILE.json`, JSON.stringify({file}))
                console.log(`INFO: '${file}' save successfull`)
            })
        } else 
            console.log('INFO: Save cancel')
    }
}
...
program
    ...
    .option('-t, --type <type-keyword>', 'input type(lyric | embed | other | auto)', config.inputType ? config.inputType : 'lyric')
    ...
    .option('--show-diff [isShowDiff]', 'show diff information', config.isShowDiff ? config.isShowDiff : false)
    .action((input, opts) => {
        try {
            if (opts.output === true) 
